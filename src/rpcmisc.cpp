...
// Copyright (c) 2017-2018 The Cloudenode developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total cloudenode balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zCDSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zCDS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zCDS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zCDS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zCDS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zCDS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zCDS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zCDS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zCDS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zCDS denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cloudenode/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cloudenode/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcdsObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcdsObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zCDSsupply", zcdsObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cloudenodeaddress\"\n"
            "\nReturn information about the given cloudenode address.\n"
            "\nArguments:\n"
            "1. \"cloudenodeaddress\"     (string, required) The cloudenode address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cloudenodeaddress\", (string) The cloudenode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Cloudenode address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cloudenode addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cloudenode address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"cloudenodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"cloudenodeaddress\"  (string, required) The cloudenode address to use for the signature.\n"
