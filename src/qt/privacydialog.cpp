                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCDS ought to be enough for anybody." - Bill Gates, 2017
    ui->zCDSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCDSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCDS->setEnabled(false);
        ui->pushButtonMintzCDS->setToolTip(tr("zCDS is currently disabled due to maintenance."));

        ui->pushButtonSpendzCDS->setEnabled(false);
        ui->pushButtonSpendzCDS->setToolTip(tr("zCDS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCDSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCDS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCDS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCDS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCDS in ") +
...
void PrivacyDialog::on_pushButtonSpendzCDS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCDS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCDS
        sendzCDS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCDS
    sendzCDS();
}

void PrivacyDialog::on_pushButtonZCdsControl_clicked()
{
    ZCdsControlDialog* zCdsControl = new ZCdsControlDialog(this);
    zCdsControl->setModel(walletModel);
    zCdsControl->exec();
}

void PrivacyDialog::setZCdsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCdsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCDS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cloudenode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCDSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCDS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CDS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCDS</b>";
    ...
    // use mints from zCds selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCdsControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCdsControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcds selector in case it was used
    ZCdsControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCds Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCDS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCDS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCDS "));
    ui->labelzCDSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCDSSyncStatus->setVisible(fShow);
}
