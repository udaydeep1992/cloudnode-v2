...
#include "ui_zcdscontroldialog.h"
...
std::list<std::string> ZCdsControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCdsControlDialog::listMints;

ZCdsControlDialog::ZCdsControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCdsControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCdsControlDialog::~ZCdsControlDialog()
{
    delete ui;
}

void ZCdsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCdsControlDialog::updateList()
{
...
void ZCdsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCdsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCds_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCdsControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCdsControlDialog::GetSelectedMints()
{
...
void ZCdsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
