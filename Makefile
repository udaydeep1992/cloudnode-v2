...
pkgdatadir = $(datadir)/cloudenode
pkgincludedir = $(includedir)/cloudenode
pkglibdir = $(libdir)/cloudenode
pkglibexecdir = $(libexecdir)/cloudenode
...
CONFIG_HEADER = $(top_builddir)/src/config/cloudenode-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/cloudenode-config.h.in \
...
ACLOCAL = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = cloudenode-cli
BITCOIN_DAEMON_NAME = cloudenoded
BITCOIN_GUI_NAME = cloudenode-qt
BITCOIN_TX_NAME = cloudenode-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = cloudenode
PACKAGE_BUGREPORT = www.cloudenode.org
PACKAGE_NAME = Cloudenode Core
PACKAGE_STRING = Cloudenode Core 3.0.0
PACKAGE_TARNAME = cloudenode
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /mnt/d/ubuntu16/cds-v2-fix
abs_srcdir = /mnt/d/ubuntu16/cds-v2-fix
abs_top_builddir = /mnt/d/ubuntu16/cds-v2-fix
abs_top_srcdir = /mnt/d/ubuntu16/cds-v2-fix
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /mnt/d/ubuntu16/cds-v2-fix/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_cloudenode_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_cloudenode_coverage.info test_cloudenode.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/CLOUDENODE-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/cloudenode-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/cloudenode-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/cloudenode-config.h
$(top_srcdir)/src/config/cloudenode-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/cloudenode-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/CLOUDENODE-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CLOUDENODE-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/CLOUDENODE-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_cloudenode.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_cloudenode -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_cloudenode_filtered.info: test_cloudenode.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_cloudenode_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_cloudenode_coverage.info: baseline_filtered_combined.info test_cloudenode_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cloudenode_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_cloudenode_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_cloudenode_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_cloudenode.coverage/.dirstamp:  test_cloudenode_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_cloudenode.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_cloudenode.coverage/ total.coverage/ $(OSX_APP)
